package org.firstinspires.ftc.teamcode.Swerb506.utility.misc;
...
public class DashboardUtil {
    ...
    public static void drawPoseHistory(Canvas canvas, List<org.firstinspires.ftc.teamcode.Swerb506.utility.math.geometry.Pose2d> poseHistory) {
        double[] xPoints = new double[poseHistory.size()];
        double[] yPoints = new double[poseHistory.size()];
        for (int i = 0; i < poseHistory.size(); i++) {
            org.firstinspires.ftc.teamcode.Swerb506.utility.math.geometry.Pose2d pose = poseHistory.get(i);
            xPoints[i] = pose.getX();
            yPoints[i] = pose.getY();
        }
    ...
    public static void drawSampledPath(Canvas canvas, PathPlannerTrajectory path, double resolution) {
        ...
        for (int i = 0; i < samples; i++) {
            ...
            org.firstinspires.ftc.teamcode.Swerb506.utility.math.geometry.Pose2d pose = path.sample(displacement).getTargetHolonomicPose();
            xPoints[i] = pose.getX();
            yPoints[i] = pose.getY();
        }
        canvas.strokePolyline(xPoints, yPoints);
